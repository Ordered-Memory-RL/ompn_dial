...
from utils import get_env_encoder, DictList, Normaliser, get_action_dist
import numpy as np
...
class CompILE(nn.Module):
    ...
    def __init__(self, vec_size, hidden_size, action_size,
                 env_arch, max_num_segments, dataloader, latent_dist='discrete',
        ...
        self.action_emb = nn.Linear(action_size, hidden_size)
    ...
    def compute_loss(self, batch, all_b, all_masks, all_recs, all_z):
        # Compute loss
        inp = self.anorm.normalize(batch.actions.float())
        targets = inp
        ll = 0.
        kl_z = 0.
        for seg_id in range(self.max_num_segments):
            ...
            mean = all_recs[seg_id]
            dist = get_action_dist(mean)
            logprobs = dist.log_prob(targets)

            ll += (logprobs[:, :-1] * seg_prob[:, :-1]).sum(1).mean(0)
            # KL divergence on z.
        ...
        loss = -ll + self.beta_z * kl_z + self.beta_b * kl_b
        return {'loss': loss, 'nll': -ll, 'kl_b': kl_b, 'kl_z': kl_z}

    def forward(self, batch, lengths, sketch_lens):
        # Encoding inputs
        embeddings, obs_emb, env_emb = self.encode_obs(batch, sketch_lens)
        ...
        for seg_id in range(self.max_num_segments):
            ...
            reconstructions = self.decode(obs_emb, env_emb, sample_z)
    ...
    def encode_obs(self, obs: DictList, sketch_lens):
        obs_emb = self.obs_enc(self.snorm.normalize(obs.states.float()))
        seqlen = obs_emb.shape[1]
        env_emb = self.env_emb(obs.tasks, sketch_lens)
        action_emb = self.action_emb(self.anorm.normalize(obs.actions.float()))
        combined_input = torch.cat([obs_emb, env_emb[:, None, :].repeat(1, seqlen, 1), action_emb], dim=-1)
        return self.input_encoder(combined_input), obs_emb, env_emb
    ...
    def decode(self, obs_emb, env_emb, sample_z):
        """Decode single time step from latents and repeat over full seq."""
        seqlen = obs_emb.shape[1]
        comb_input = torch.cat([obs_emb, env_emb[:, None, :].repeat(1, seqlen, 1),
                                sample_z[:, None, :].repeat(1, seqlen, 1)], dim=-1)
